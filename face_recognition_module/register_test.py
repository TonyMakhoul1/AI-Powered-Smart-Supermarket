...
if not firebase_admin._apps:
    cred = credentials.Certificate(
        'smart_supermarket_project/face_recognition_module/smart-supermarket-project-firebase-adminsdk-fbsvc-7e87b3b077.json')
...
def is_duplicate_face(new_encodings):
    ...
    for filename in os.listdir(ENCODING_DIR):
        if filename.endswith(".pkl"):
            existing_name = filename[:-4]
            file_path = os.path.join(ENCODING_DIR, filename)
            with open(file_path, 'rb') as file:
                existing_encodings = pickle.load(file)

            matches = face_recognition.compare_faces(
                existing_encodings, new_encodings, tolerance=FACE_MATCH_TOLERANCE)
            if True in matches:
...
def cleanup_registration_files(file_path, person_image_dir):
    if file_path and os.path.exists(file_path):
        os.remove(file_path)
        print("Deleted local encoding file due to registration failure.")

    if person_image_dir and os.path.exists(person_image_dir):
        shutil.rmtree(person_image_dir)
        print("Deleted saved images due to registration failure.")
...
def register_customer(name, phone, progress_callback=None):
    file_path = None
    ...
    lock_face = None  # to store the first face location
    ...
    try:
        ...
        if len(encodings) == max_samples:
            ...
            print("Succefully saved face Encodings.")
        ...
        # if not registration_succesful:
        # cleanup_registration_files(file_path, person_image_dir)
